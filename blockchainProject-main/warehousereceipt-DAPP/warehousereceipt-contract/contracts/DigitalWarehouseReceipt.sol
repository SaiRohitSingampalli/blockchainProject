...
contract DigitalWarehouseReceipt is ERC721, ERC721URIStorage, AccessControl, Ownable {
    ...
    //events
    event newToken(uint256 tokenId, address seller, uint256 price);
    event receiptTradedEvent(uint256 tokenId, address buyer, uint256 price);
    event destroyReceiptEvent(uint256 tokenId, address warehouse);
    ...
    function createWarehouseReceipt(address to, string memory uri, uint256 price, string memory receiptName) public onlyRole(WAREHOUSE) {
        ...
        emit newToken(tokenId, to, price);
        //return tokenId;
    }
   ...
   function bidForReceipt(uint tokenId, uint bid) public returns (bool) { 
    ...
    function buyReceipt(uint tokenId) public payable returns (bool) {
        ...
        activeCount -= 1;
        emit receiptTradedEvent(tokenId, WarehouseReceipts[tokenId].seller, msg.value);
        return true;
    }
    ...
    function setPrice(uint tokenId, uint256 price) public onlySeller(tokenId) returns (uint256) {
        ...
        return WarehouseReceipts[tokenId].price;
    }
    ...
    function destroyToken(uint tokenId) public onlyRole(WAREHOUSE) returns (bool) {
        ...
        _burn(tokenId);
        emit destroyReceiptEvent(tokenId, WarehouseReceipts[tokenId].warehouse);
        return true;
    }
